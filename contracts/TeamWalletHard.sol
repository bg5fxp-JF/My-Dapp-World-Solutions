...
contract TeamWallet {
    ...
    function transactionStats() public view onlyMember returns (uint debitedCount,uint pendingCount,uint failedCount){
        for (uint i; i < transactionId; i++ ) {
            if(keccak256(bytes(transactionIdToTransaction[i].status)) == keccak256((bytes("debited")))) debitedCount++;
            if(keccak256(bytes(transactionIdToTransaction[i].status)) == keccak256((bytes("pending")))) pendingCount++;
            if(keccak256(bytes(transactionIdToTransaction[i].status)) == keccak256((bytes("failed")))) failedCount++;
        }
    }
